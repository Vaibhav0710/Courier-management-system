...
import { createTwoWheelerOrder } from "../Services/user.service";
...
const TwoWheelerPage = () => {
  ...
  const calculateFare = (distance) => (distance * 20).toFixed(0);

  const fetchCoordinates = async (address) => {
    const url = `https://graphhopper.com/api/1/geocode?q=${encodeURIComponent(address)}&limit=1&key=${API_KEY}`;
    try {
  ...
  const fetchDistance = async () => {
    ...
    if (pickupCoords && deliveryCoords) {
      const url = `https://graphhopper.com/api/1/route?point=${pickupCoords[1]},${pickupCoords[0]}&point=${deliveryCoords[1]},${deliveryCoords[0]}&vehicle=bike&key=${API_KEY}`;
      try {
        const response = await fetch(url);
        const data = await response.json();
        ...
        if (distanceInKm) {
          ...
          toast.success(`Distance: ${distanceInKm} km`);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createTwoWheelerOrder(bookingData);
      toast.success("Two-wheeler booking successful!");
    } catch (err) {
  ...
  return (
    <Container className="truck-page-container mt-5">
      <NaavBaar />
      <h1 className="text-center truck-page-heading">Two-Wheeler Services</h1>
      <section className="booking-section">
        <h2 className="booking-heading">Book a Two-Wheeler</h2>
